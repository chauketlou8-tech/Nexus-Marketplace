...
export default function SearchBar({ search, setSearch }: SearchProps) {
    ...
    return (
        <div className="flex justify-center items-center relative w-[70%]">
            ...
            <input onChange={(e) => setSearch?.(e.target.value)} type="text" placeholder="Search textbooks, items, services..." className="w-full bg-black border-1 border-gray-200 outline-0 rounded-[5px] py-2 pl-8 pr-3 text-[12px] text-white"/>
        </div>
    )
}